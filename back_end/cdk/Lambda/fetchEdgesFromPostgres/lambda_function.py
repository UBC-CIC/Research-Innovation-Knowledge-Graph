...
def lambda_handler(event, context):
    ...
    if len(event["arguments"]["facultiesToFilterOn"]) > 0 and len(event["arguments"]["keyword"]) > 0:
        ...
        keywordQueryString, keywordsToAdd = createKeywordsString(event["arguments"]["keyword"].split(", "))
        ...
        cursor.execute("SELECT * FROM public.edges_full WHERE source_id=ANY(SELECT scopus_id FROM researcher_data WHERE prime_faculty=ANY(%s)) AND target_id=ANY(SELECT scopus_id FROM researcher_data WHERE prime_faculty=ANY(%s))", (event["arguments"]["facultiesToFilterOn"], event["arguments"]["facultiesToFilterOn"],))
